...
1. `#{}`：
   - 这种方式主要用于预编译SQL语句（PreparedStatement）。它会将参数值作为一个完整的单元插入到SQL语句中，并且会对参数进行适当的转义处理，防止SQL注入。
   - `#{}` 会根据参数类型自动添加必要的引号（对于字符串类型）。例如，如果参数是一个字符串，它会被转换成 `'value'` 这样的形式。
   - `#{}` 不会直接拼接参数值到SQL语句中，而是通过占位符（如 `?`）的方式在执行时传递参数。
2. `${}`：
   - 这种方式主要用于字符串拼接。它会直接将参数值插入到SQL语句中，就像你在写普通的字符串一样。
   - `${}` 没有预编译的优势，因此容易受到SQL注入攻击。它适用于那些你确实需要将参数值直接插入到SQL语句中的场景，比如动态构建表名或列名。

在使用`LIKE`子句时，通常推荐使用`#{}`来传递参数，因为它能够帮助预防SQL注入问题。例如，如果你想构建一个`LIKE`查询，你可以这样做：
